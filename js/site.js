...
async function play(params) {
  try {
    ...
    OmniBox.log("info", `获取播放地址1: playId=${playId}, flag=${flag}, videoId=${videoId}`);

    // 构建播放地址响应
    // 统一使用数组格式，每个元素包含 name 和 url，类似 danmaku 格式
    let newUrl = playId;
    if (flag == "ruyi") {
      OmniBox.log("info", "开始嗅探地址" + playId);
      const { url, header } = await OmniBox.sniffVideo(playId);
      newUrl = url;
      OmniBox.log("info", url);
    }
    let urlsResult = [
      {
        name: "播放",
        ...
        header: {
          ...
          Referer: "https://svip.ryplay16.co/",
        },
      },
    ];
