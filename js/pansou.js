...
const PANSOU_CLOUD_TYPES = process.env.PANSOU_CLOUD_TYPES || "quark";
...
async function home(params, context) {
  OmniBox.log("info", `from参数: ${params.from}`);
  try {
    ...
    let list = [];
    try {
      const categoryData = await OmniBox.getSourceCategoryData("history", 1, 20);
...
async function detail(params, context) {
  try {
    ...
    const sourceId = `spider_source_${context.sourceId}_${shareURL}`;
    try {
...
async function play(params, context) {
  try {
    ...
    if (parts.length < 2) {
      throw new Error("播放参数格式错误，应为：分享链接|文件ID" + playId);
    }
    const shareURL = parts[0] || "";
    ...
    try {
      const resourceId = `spider_source_${context.sourceId}_${shareURL}`;
